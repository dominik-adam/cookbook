...
type IngredientsProps = {
  ...
  initSliderState: number;
  initIngredientState?: string;
};
...
export default function Ingredients({
  ...
  const [slider, setSlider] = useState<number>(initSliderState);
  const [ingredientState, setIngredientState] = useState<string>(initIngredientState ?? Array(ingredients.length).fill('0').join(''));

  const handleSliderChange = (value: string) => {
    setSlider(parseInt(value));
  }
