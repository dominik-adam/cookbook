...
import type { Ingredient as IngredientType, IngredientUnit } from '@/types/recipe';
...
type IngredientProps = {
  ...
  multiplier?: number;
  isChecked: boolean;
  handleToggle?: (ingredientId: string, isChecked: boolean) => void;
};
...
export default function Ingredient({
  ...
  const nutrition = calculateIngredientNutrition(ingredient, (amount ?? 0) * multiplier, unitName);

  const [wasAdded, setWasAdded] = useState<boolean>(false);

  const addToBag = async () => {
    setWasAdded(true);
    const response = await addIngredientToBag({ ingredientId, unitId, amount: (amount ?? 0) * multiplier });
    if (response.ok) {
