...
type BagIngredientProps = {
  ingredient: Ingredient;
  amount: number | null;
  unit: IngredientUnit;
  note: string | null;
  handleRemove: (ingredientId: string, ingredientName: string, unitId: string) => void;
};
...
export default function BagIngredient({
  ...
  const [amount, setAmount] = useState<number | null>(initAmount);
  const [note, setNote] = useState<string | null>(initNote);
  const [newAmount, setNewAmount] = useState<number | null>(initAmount);
  const [newNote, setNewNote] = useState<string | null>(initNote);

  const [isHover, setIsHover] = useState<boolean>(false);
  const [isEdit, setIsEdit] = useState<boolean>(false);

  const editBagIngredient = async () => {
    const response = await updateBagIngredient({ingredientId, unitId, amount: newAmount, note: newNote ?? undefined});
    if (response.ok) {
